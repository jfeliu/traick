...
async def admin_dashboard(request: Request):
    ...
    return templates.TemplateResponse(request=request, name="dashboard.html", context={"project_count": count})
...
async def create_project(request: Request, owner_number: str = Form(...), name: str = Form(...), description: str = Form("")):
    async with aiosqlite.connect(settings.db_path) as db:
        await db.execute(
            """
...
async def edit_project_form(request: Request, project_id: int):
    ...
    return templates.TemplateResponse(request=request, name="project_form.html", context={"project": project})

@router.post("/projects/{project_id}/edit")
async def update_project(request: Request, project_id: int, owner_number: str = Form(...), name: str = Form(...), description: str = Form("")):
    async with aiosqlite.connect(settings.db_path) as db:
        await db.execute(
            """
        ...
        await db.commit()
    return RedirectResponse(f"/admin/projects", status_code=303)
...
async def list_action_items(request: Request):
    async with aiosqlite.connect(settings.db_path) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("SELECT ai.*, p.name as project_name FROM action_items ai LEFT JOIN projects p ON ai.project_id = p.id ORDER BY ai.created_at DESC")
        action_items = await cursor.fetchall()
    return templates.TemplateResponse(request=request, name="action_items.html", context={"action_items": action_items})
...
async def new_action_item_form(request: Request):
    async with aiosqlite.connect(settings.db_path) as db:
        ...
        projects = await cursor.fetchall()
    return templates.TemplateResponse(request=request, name="action_item_form.html", context={"projects": projects})

@router.post("/action_items/new")
async def create_action_item(request: Request, project_id: int = Form(...), description: str = Form(...), deadline: str = Form("")):
    async with aiosqlite.connect(settings.db_path) as db:
        await db.execute(
            """
...
async def edit_action_item_form(request: Request, action_item_id: int):
    async with aiosqlite.connect(settings.db_path) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("SELECT * FROM action_items WHERE id = ?", (action_item_id,))
        action_item = await cursor.fetchone()
        cursor = await db.execute("SELECT id, name FROM projects ORDER BY name")
        projects = await cursor.fetchall()
    if not action_item:
        raise HTTPException(status_code=404, detail="Action item not found")
    return templates.TemplateResponse(request=request, name="action_item_form.html", context={"action_item": action_item, "projects": projects})

@router.post("/action_items/{action_item_id}/edit")
async def update_action_item(request: Request, action_item_id: int, project_id: int = Form(...), description: str = Form(...), deadline: str = Form("")):
    async with aiosqlite.connect(settings.db_path) as db:
        await db.execute(
            """
...
async def list_follow_ups(request: Request):
    async with aiosqlite.connect(settings.db_path) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("SELECT fu.*, p.name as project_name, ai.description as action_item_desc FROM follow_ups fu LEFT JOIN projects p ON fu.project_id = p.id LEFT JOIN action_items ai ON fu.action_item_id = ai.id ORDER BY fu.scheduled_at DESC")
        follow_ups = await cursor.fetchall()
    return templates.TemplateResponse(request=request, name="follow_ups.html", context={"follow_ups": follow_ups})
...
async def create_follow_up(request: Request, project_id: int = Form(...), action_item_id: int = Form(...), message: str = Form(...), scheduled_at: str = Form(...)):
    async with aiosqlite.connect(settings.db_path) as db:
        await db.execute(
            """
...
async def edit_follow_up_form(request: Request, follow_up_id: int):
    ...
    return templates.TemplateResponse(request=request, name="follow_up_form.html", context={"follow_up": follow_up, "projects": projects, "action_items": action_items})

@router.post("/follow_ups/{follow_up_id}/edit")
async def update_follow_up(request: Request, follow_up_id: int, project_id: int = Form(...), action_item_id: int = Form(...), message: str = Form(...), scheduled_at: str = Form(...)):
    async with aiosqlite.connect(settings.db_path) as db:
        await db.execute(
            """
...
async def db_overview(request: Request):
    ...
    return templates.TemplateResponse(request=request, name="db_overview.html", context={"tables": tables, "table_counts": table_counts})
